...
function loop(counter, condition, updateCounter, loopBody) {
  let _counter = counter;

  while(condition(_counter)) {
    loopBody(_counter);
    _counter = updateCounter(_counter);
  }
}
...
// Using some
function every(array, predicate) {
  // If some do not satisfy the predicate then not every element satisfy the predicate
...
function dominantDirection(text) {
  ...
  let dominant = { name: 'unknown', count: -Infinity };
  for (const group of counts) {
    if (group.count > dominant.count) {
      dominant = group;
    }
  }
